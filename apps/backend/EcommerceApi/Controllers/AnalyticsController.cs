    ...
    public class AnalyticsController : ControllerBase
    {
            ...
            try
            {
                ...
                var productPerformance = products
                    .Select(p => new ProductPerformanceDto
                    {
                        ...
                        TotalRevenue = p.Variants.SelectMany(v => orders.SelectMany(o => o.Items).Where(oi => oi.VariantId == v.Id).Select(oi => oi.Price * (decimal)oi.Quantity)).Sum(),
            ...
            [FromQuery] DateTime? toDate)
        {
            try
            {
                ...
                var metrics = categories.Select(c => new
                {
                    CategoryId = c.Id,
                    CategoryName = c.Name,
                ...
                var categorySales = metrics.Select(m => new CategorySalesMetricsDto
                {
                    ...
                    TotalRevenue = m.Items.Sum(i => i.Price * (decimal)i.Quantity),
                    Percentage = totalRevenue > 0 ? (m.Items.Sum(i => i.Price * (decimal)i.Quantity) * 100m) / totalRevenue : 0
                })
                .OrderByDescending(x => x.TotalRevenue)
                .ToList();
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                var orders = await _context.Orders.Where(o => o.CreatedAt >= thirtyDaysAgo).Include(o => o.Items).ToListAsync();
                ...
                var metrics = variants.Select(v => new
                {
                    Variant = v,
                    RecentSales = orders.SelectMany(o => o.Items).Count(oi => oi.VariantId == v.Id),
                    SalesValue = orders.SelectMany(o => o.Items).Where(oi => oi.VariantId == v.Id).Sum(oi => oi.Price * (decimal)oi.Quantity)
                }).ToList();
            ...
            [FromQuery] int threshold = 10)
        {
            try
            {
                ...
                var alerts = variants.Select(v => new
                {
                    Variant = v,
                    RecentSalesCount = orders.SelectMany(o => o.Items).Count(oi => oi.VariantId == v.Id)
                }).ToList();
            ...
            [FromQuery] DateTime? toDate)
        {
            try
            {
                ...
                var vendorMetrics = vendors.Select(v => new
                {
                    Vendor = v,
                ...
                var performance = vendorMetrics.Select(m => new VendorPerformanceDto
                {
                    ...
                    AverageRating = m.Vendor.Products.SelectMany(p => p.Reviews).Count() > 0
                        ...
                        : 0,
                    RevenuePercentage = totalRevenue > 0 ? (m.Items.Sum(i => i.Price * (decimal)i.Quantity) * 100m) / totalRevenue : 0
                })
                .OrderByDescending(x => x.TotalRevenue)
                .ToList();
